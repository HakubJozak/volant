...
module Export
  ...
  #                                                 }
      ...
      module ClassMethods
        ...
        CSV_COLUMNS = [:id, :code, :name,
                       :country_code, :country_name,
                       :org_code, :org_name, :org_networks,
                       :language, :from,:to, :capacity, :places,
                       :places_for_males, :places_for_females,
                       :minimal_age, :maximal_age, :area, :accomodation,
                       :workdesc, :notes, :description, :created_at,
                       :updated_at, :extra_fee, :extra_fee_currency,
                    :region, :capacity_natives, :capacity_teenagers,
                    ...
                    :asked_for_places_females, :type, :sci_code,
                    :longitude, :latitude, :state, :sci_id,
                    :requirements, :free_places,
                    :free_places_for_males, :free_places_for_females,
                    :project_id, :duration, :free_capacity_males,
        ...
        def to_csv
          ::CSV.generate(:col_sep => ';') do |csv|
            ...
            find_each do |wc|
              values = CSV_COLUMNS.map do |attr|
                case attr
                when :from, :to
                  if val = wc.send(attr)
                    I18n.l(val.to_date)
