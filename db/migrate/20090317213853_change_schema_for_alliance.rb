...
class ChangeSchemaForAlliance < ActiveRecord::Migration

  def self.up
    CreateAdvancedWorkcampView.down
    change_column :workcamps, :minimal_age, :integer, :null => true
    change_column :workcamps, :maximal_age, :integer, :null => true
    add_column :workcamps, :extra_fee, :decimal, :scale => 2, :precision => 10
    add_column :workcamps, :extra_fee_currency, :string
    add_column :workcamps, :region, :string
    CreateAdvancedWorkcampView.up

    add_column :countries, :triple_code, :string, :limit => 3
    Country.reset_column_information

    [['AF','AFG'], ['ALB','ALB'], ['DZ','DZA'],['AS','ASM'], ['AD','AND'],
     ...
     ['CC','CCK'], ['CO','COL'], ['KM','COM'], ['CG','COG'], ['CK','COK'],
     ['CR','CRI'], ['CI','CIV'], ['HR','HRV'], ['CU','CUB'], ['CY','CYP'],
     ['CZ','CZE'], ['DK','DNK'], ['DJ','DJI'], ['DM','DMA'], ['DO','DOM'],
     ['TP','TMP'], ['EC','ECU'], ['EG','EGY'], ['SV','SLV'], ['GQ','GNQ'],
     ['ER','ERI'], ['EE','EST'], ['ET','ETH'], ['FK','FLK'], ['FO','FRO'],
     ['FJ','FJI'], ['FI','FIN'], ['FR','FRA'], ['FX','FXX'], ['GF','GUF'],
     ...
     ['ZM','ZMB'],
     ['ZW','ZWE']
    ].each do |double,triple|
      c = Country.find_by_code!(double) rescue say("Country #{double} not present.")
      c.triple_code = triple and c.save if c
    end
  end

  def self.down
    remove_column :countries, :triple_code

    CreateAdvancedWorkcampView.down
    remove_column :workcamps, :extra_fee
    remove_column :workcamps, :extra_fee_currency
    remove_column :workcamps, :region
    CreateAdvancedWorkcampView.up
  end
end
